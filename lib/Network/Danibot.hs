...
-}
dumbHandler :: (Text -> UserMessage -> IO Text,Description)
dumbHandler = 
    let go _ _ = do 
            threadDelay 0.5e6
...
-}
lookupHandler :: Map Text Text -> (Text -> UserMessage -> IO Text,Description) 
lookupHandler aMap = 
    let go key _ = pure (maybe "Entry not found." id (Map.lookup key aMap))
    in  (go,"key") 

{-| Is the given port listening on the given host?		

-}
isUpHandler :: (Text -> UserMessage -> IO Text,Description)
isUpHandler =
    let go (Text.break isSpace . Text.strip -> (Text.strip -> host,Text.strip -> port)) _ = do 
...
-}
helpHandler :: [(Command,(Text -> UserMessage -> IO Text,Description))] -> (Text -> UserMessage -> IO Text, Description)
helpHandler handlers = 
    let helptext = mconcat (intersperse "\n" [k <> " " <> v | (k,(_,v)) <- handlers])
    in  (\_ _ -> pure helptext,"")
...
-}
dispatch :: [(Command,(Text -> UserMessage -> IO Text,x))] -> Text -> UserMessage -> IO Text
dispatch (Map.fromList -> handlers) (splitWord -> (key,request)) userMessage =   
    case Map.lookup key handlers of
        Just (f,_) -> f request userMessage
